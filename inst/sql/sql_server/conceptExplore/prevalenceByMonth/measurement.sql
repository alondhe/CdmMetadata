...
(
  ...
  FROM
    (SELECT *
     FROM @resultsDatabaseSchema.ACHILLES_results WHERE analysis_id = 1802) num
    INNER JOIN
    (SELECT *
     FROM @resultsDatabaseSchema.ACHILLES_results WHERE analysis_id = 117) denom ON num.stratum_2 = denom.stratum_1
    --calendar year
    INNER JOIN @vocabDatabaseSchema.concept c1 ON CAST(num.stratum_1 AS INT) = c1.concept_id
